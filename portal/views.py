...
from portal.authhelper import get_signin_url
from portal.authhelper import get_token_from_code
# from portal.authhelper import get_access_token
from portal.outlookservice import get_me
...
def home(request):
    if request.user.is_authenticated:
        ...
        redirect_uri = request.build_absolute_uri(reverse('portal:gettoken'))
        # return HttpResponse(redirect_uri)
        sign_in_url = get_signin_url(redirect_uri)
        # return HttpResponse('<a href="' + sign_in_url +'">Click here to sign in and view your mail</a>')
        return render(request ,'portal/login.html',{'sign_in_url' : sign_in_url})
...
def gettoken(request):
    ...
    outlook_user = get_me(access_token)
    webmail = outlook_user['mail']
    if 'iitg.ac.in' in webmail:
        webmail = webmail.rstrip('@iitg.ac.in')
        ...
        if username is not None:
            username.set_password('model_password')
            username.save()
            user = authenticate(username = username , password = 'model_password')
            login(request, user)
            if user is not None:
            ...
            new_user = User.objects.create_user(username = webmail, password='model_password',email=email)
            new_user.save()
            user = Profile.objects.get(user =  new_user.id)
            user.webmail = webmail
            user.save()
            log_user = authenticate(username=webmail, password='model_password')
            login(request, log_user)
        return HttpResponseRedirect('/portal/pending/')
    else:
        return HttpResponse('You are not authenticated to use this portal,Kindly login from IITG webmail')
    # Save the token in the session
    # request.session['access_token'] = access_token
    # return HttpResponse('User: {0}, Access token: {1} , email: {2}'.format(user['displayName'], access_token,user['mail']))
...
def test(request):
    obj = request_progress.objects.filter(request_user_webmail=request.user.profile.webmail, approved=True).order_by('-request_year')

    return render(request,'portal/certi_page.html' , { 'post': obj })
